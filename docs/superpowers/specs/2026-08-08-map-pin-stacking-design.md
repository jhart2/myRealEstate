...
- **Zoom ≥ 17:** fan the group upward (`n × STACK_PX` ≈ 30px) via `iconAnchor` — stored lat/lng stay true
- **Zoom < 17:** show only the most relevant pin for that point (first in `listingsValue` / current search sort); hide the rest (`opacity: 0`, non-interactive)
