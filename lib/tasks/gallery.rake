namespace :gallery do
  ...
  task backfill: :environment do
    ...
    only_needed = ENV["FORCE"].to_s !~ /\A(1|true|yes)\z/i
    ...
    puts "Gallery ingest backfill (#{mode}, only_needed=#{only_needed}, limit=#{limit || "all"})…"
      ...
      if inline
        begin
          ...
          puts "  [#{queued}] ##{property.id} #{property.bok_id || property.slug} " \
               "attached=#{result[:attached]} skipped=#{result[:skipped]} " \
               "errors=#{result[:errors].size} (#{urls.size} urls)"
    ...
    puts "Done. scanned=#{scanned} processed=#{queued} skipped=#{skipped} " \
         "attached=#{attached} errors=#{errors} elapsed=#{elapsed}s"
    unless inline
