...
echo "Gallery staging backfill → Cloud SQL + gs://${GCS_BUCKET} (INLINE=1, GALLERY_ENHANCE=${GALLERY_ENHANCE:-1})…"
cd "$ROOT"
# Pass through LIMIT / BOK_ID / FORCE / ONLY_NEEDED / GALLERY_ENHANCE* / ESRGAN* from caller.
env \
  ...
  GALLERY_ENHANCE="${GALLERY_ENHANCE:-1}" \
  GALLERY_ENHANCE_ESRGAN="${GALLERY_ENHANCE_ESRGAN:-1}" \
  INLINE=1 \
